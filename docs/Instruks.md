...
и хочет собрать и запустить приложение-конструктор **NovaJava Studio**,
...
**Конструктор нативных Android-приложений на Java + XML**, который сам живёт
на Android. Проекты собираются **кастомным пайплайном без Gradle**
(`aapt2 → javac → d8 → zipalign → apksigner`) — это секунды вместо минут:
...
1. На главном экране — **«Новый проект»**: имя и Application ID
   (например `MyApp`, `com.example.myapp`). Файлы проекта **Java + XML**
   создаются мгновенно — без скачиваний и установки зависимостей.
2. Проект создаётся в `/root/projects/<имя>` внутри Ubuntu; вкладка
   **«Редактор»** открывает макет экрана (`app/res/layout/activity_main.xml`),
   код Activity (`app/src/**/MainActivity.java`) — через «Файлы проекта» слева.
   Всё сохраняется на диск как есть.
3. Вкладка **«Дизайн / Превью»** — горячий предпросмотр макета из текущего
   кода: сохранили файл — превью перерисовалось (это и есть hot reload).
   Экран **«Горячий предпросмотр»** следит за файлом сам и умеет
   «Собрать и установить».
4. Вкладка **«Сборка»** → задача **Debug APK** (или Release / AAB / Ключ /
   Очистка) → «Запустить»:
   - внутри — кастомный сборщик **Storm Build** (`bash build.sh` →
     `storm build apk|aab`): `aapt2 → javac → d8/R8 → zipalign → apksigner`,
     **без Gradle**;
   - сборка **идёт в фоне** — можно свернуть приложение;
   - готовый APK копируется в **`/sdcard/Download/NovaJava/<проект>/apk`**
     (и остаётся в `build/outputs/` проекта);
   - для публикации: сначала «Ключ подписи», затем **Release APK** или **AAB**;
   - Maven-библиотеки — экран «Пакеты» (`storm deps add …`).
5. Кнопка **«Установить APK»** на экране сборки — установка с подтверждением
